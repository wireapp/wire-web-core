...
describe('Message', () => {
  const sessionTag = new SessionTag();
  sessionTag.tag.fill(42);

  const baseKey = new PublicKey(
    new Uint8Array([
      226, 201, 222, 33, 39, 107, 40, 144, 176, 61, 100, 172, 187, 75, 226, 187, 56, 224, 93, 172, 160, 59, 23, 5, 89,
      135, 147, 150, 186, 35, 251, 244,
    ]),
    new Uint8Array([
      239, 255, 64, 169, 64, 181, 174, 73, 140, 59, 238, 132, 78, 180, 124, 168, 31, 86, 109, 137, 143, 133, 99, 134,
      196, 21, 48, 194, 228, 57, 143, 97,
    ]),
  );
  const ratchetKey = new PublicKey(
    new Uint8Array([
      133, 117, 192, 130, 228, 136, 206, 141, 238, 5, 132, 167, 121, 244, 82, 240, 233, 26, 205, 137, 188, 232, 29, 172,
      208, 55, 162, 118, 135, 79, 197, 15,
    ]),
    new Uint8Array([
      136, 242, 230, 94, 41, 142, 44, 0, 181, 172, 69, 159, 61, 246, 118, 230, 28, 211, 115, 255, 177, 61, 31, 160, 108,
      154, 194, 3, 208, 25, 139, 63,
    ]),
  );
  const publicIdentityKey = new PublicKey(
    new Uint8Array([
      159, 59, 98, 66, 70, 188, 133, 198, 240, 219, 174, 199, 124, 130, 51, 101, 242, 243, 137, 16, 44, 8, 130, 189,
      245, 168, 190, 133, 108, 42, 103, 126,
    ]),
    new Uint8Array([
      222, 129, 110, 54, 26, 250, 178, 102, 255, 166, 210, 24, 142, 190, 231, 7, 140, 141, 253, 162, 102, 219, 176, 11,
      169, 131, 39, 12, 103, 74, 232, 44,
    ]),
  );
  const identityKey = new IdentityKey(publicIdentityKey);
  ...
  it('serialises a PreKeyMessage correctly', () => {
    ...
    const cipherMessage = new CipherMessage(
      sessionTag,
      12,
      13,
      ratchetKey,
    ...
    const preKeyMessage = new PreKeyMessage(24, baseKey, identityKey, cipherMessage);
